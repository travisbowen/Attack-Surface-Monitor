...
These pin the CURRENT scoring arithmetic. They are deliberately explicit about
the individual point contributions so that changing a weight fails loudly with
a readable diff, rather than silently shifting every report's ordering.
"""
...
def test_findings_are_returned_highest_risk_first():
    ...
    assert [f["risk_score"] for f in scored] == [74, 15, 0]
